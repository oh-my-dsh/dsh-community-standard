...
| 字段 | 内容 |
| --- | --- |
| 反馈来源 | [omdsh-dev/community#24](https://github.com/omdsh-dev/community/issues/24)（v0.1 定稿 + 第三轮 5 条评论） |
| 处置版本 | v0.15 |
| 状态 | 已归档 |
...
| URL query 状态持久化/回填（类 Grafana dashboard，应对桌面随机端口）（morlay） | **限定采纳** | 归 Web Presentation capability（暂名 `x-web.panel.urlState`），带字段白名单、大小上限、按插件隔离、禁存 secret；不进核心，不强加给 TUI / headless 宿主（采 Qiuner 建议） |
