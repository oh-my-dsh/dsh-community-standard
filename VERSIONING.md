...
这份文件管一件事：这个标准里所有的"版本号"分别是什么意思、什么时候变、变了谁受影响。这是整个标准最容易被搞混的地方，所以单独一份。**插件作者、宿主维护者、工具作者都该读**——读完你应该能回答："我要改的这个东西，该动哪个版本号？"
...
- **兼容判断只看 2、3、4，不看 5。** 宿主兼不兼容取决于它声明实现的契约坐标，而不是 `gui>=2.0` 这类产品版本号。协商规则见 [spec/negotiation.md](spec/negotiation.md)。
...
- **契约的 breaking change 必须发布新坐标**（新 `apiVersion` 或新 kind），不得在旧坐标下原地修改语义。新坐标登记走 [RFC 0000](rfcs/0000-governance.md) 规定的 RFC 流程。
