...
词条模板（每个 API 按此格式写）：**签名 → 参数 → 返回值 → 错误 → 生命周期约束 → 示例 → 对应 fixture → 引入版本（since）**。
...
以下接口归各 registry 条目定义，列在此处便于查阅；冲突时以 registry 条目定稿为准。
...
同时需要一并修订的既有文档：facet-model §2.3（第三面改为 `contracts`）、registry 三条目的用法示例（改为经统一窗口获取句柄，具名属性写法标注"SDK 便利层，非标准"）。
